...
ch_sep_plot <- #ranked_upos %>%
  ggplot(ranked_upos) +
  aes(xmin = 0, 
      xmax = n) + # I think this is one of the places that's throwing an error. You've set your x-scale here from 0-5 by dividing by 100, but your limits and breaks below are running -100 - 500. So I changed xmax to "n" but I'd suggest choosing either the 1-5 or 100-500 scale and that should fix one of the issues.
  aes(ymin = rank - .45, 
      ymax = rank + .45,
      y = rank) +
  facet_wrap(~ Chapter) +
